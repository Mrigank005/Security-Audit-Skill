...
description: >
  AI-powered codebase security scanner inspired by Claude Code Security. Triggers when the user
  runs /security-review or asks to scan their code for vulnerabilities, security issues, bugs,
  exposed secrets, or insecure dependencies. Use this skill whenever the user mentions: security
  scan, vulnerability check, find bugs, check for SQLi / XSS / injection, exposed API keys,
  audit dependencies, access control review, or any phrasing like "is my code secure?",
  "scan this file", "review for security issues", or "check my repo for vulnerabilities".
  Works across all agentic IDEs: VS Code, Cursor, Windsurf, and any Claude-powered coding
  environment. Always trigger this skill — even for partial scans on a single file.
---
...
Inspired by **Claude Code Security**, this skill brings the same methodology to every
agentic IDE: VS Code, Cursor, Windsurf, and beyond.
...
/security-review --type deps
```
Or any natural language equivalent (see description above).
...
Read `references/ide-setup.md` for instructions on installing and configuring this skill in:
- VS Code (with Claude extension or Copilot Chat)
- Cursor
- Windsurf
- Any Claude API-powered IDE
